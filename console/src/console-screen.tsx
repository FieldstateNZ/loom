// The console shell: a role-scoped SideNav + TopBar wrapping the content area
// that swaps between all eight screens, with tenant/provider drill-in. Deep-
// linkable — screen/role/tenant/provider/theme sync to the URL so a refresh
// keeps your place. (The real implementation of the design bundle's
// ConsoleScreen.dc.html.)
import { useEffect, useState, type ReactNode } from "react";
import {
  SideNav, TopBar, SegmentedControl, Badge, StatusDot, Icon, Spinner, type Crumb,
} from "./components/index.ts";
import { SCREEN_TITLES, isScreenId, buildNavSections, type ScreenId, type Role, type Theme } from "./console-nav.ts";
...
/** Props for {@link ConsoleScreen}. */
export interface ConsoleScreenProps {
  /** The resolved client the shell boots from and passes to screens. */
  readonly client: LoomClient;
}

/** The stateful console shell (nav + top bar + active screen + URL sync). */
export function ConsoleScreen({ client }: ConsoleScreenProps) {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "var(--bg-0)", color: "var(--fg-1)", overflow: "hidden", font: "400 13px/1.35 var(--font-sans)" }}>
      <SideNav
        ...
        sections={buildNavSections(role)}
        footer={
